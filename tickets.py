...
def screen_forward():
    ...
    stored_data.write(
        "x{} Student \t${:.2f}\n".format(
            stdn_tickets, (stdn_tickets * costs["student"])
        )
    )
    stored_data.write(
        "x{} Pensioner \t${:.2f}\n".format(
            psr_tickets, (psr_tickets * costs["pensioner"])
        )
    )
    stored_data.write(
        "Total: ${:.2f}\n\n".format(
            (adt_tickets * costs["adult"])
            + (chd_tickets * costs["child"])
            + (stdn_tickets * costs["student"])
            + (psr_tickets * costs["pensioner"])
        )
    )
    stored_data.close()
...
class SeatType:
    ...
    def increase(self):
        """Increase ticket type within boundary."""
